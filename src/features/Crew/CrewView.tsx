...
export const CrewView = () => {
    ...
    const [newCrewDept, setNewCrewDept] = useState<Department>('Production');
                    ...
                    <div className="bg-surface border border-border-subtle rounded-xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleAddMember} className="space-y-4">
                            ...
                            <div>
                                ...
                                <select
                                    value={newCrewDept}
                                    onChange={e => setNewCrewDept(e.target.value as Department)}
                                    className="w-full px-3 py-2 bg-app border border-border-subtle rounded-lg text-primary focus:border-accent-primary outline-none"
                                >
                                    {['Production', 'Camera', 'Sound', 'Grip', 'Electric', 'Art', 'Makeup', 'Wardrobe', 'Stunts', 'Locations', 'Transportation', 'Catering'].map(d => (
                                        <option key={d} value={d}>{d}</option>
                                    ))}
                                </select>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-secondary mb-1">Daily Rate</label>
                                <input
                                    type="number"
                                    value={newCrewRate}
